 ...
 * Deal Class
 * 
 * The WPDeals deals class handles individual deals data.
 *
 * @class wpdeals_deals
 * @package		WPDeals
 * @category	Class
...
class wpdeals_deals {
	...
	function wpdeals_deals( $id ) {
		...
		// Get deals type
		$transient_name = 'wpdeals_deals_type_' . $this->id;
    ...
    function get_total_stock() {
        ...
        if (is_null($this->total_stock)) :
        	...
        	$transient_name = 'wpdeals_deals_total_stock_' . $this->id;
	...
	function get_children() {
		...
		if (!is_array($this->children)) :
			...
			if ($this->is_type('variable') || $this->is_type('grouped')) :
				...
				$transient_name = 'wpdeals_deals_children_ids_' . $this->id;
	...
	function get_child( $child_id ) {
		if ($this->is_type('variable')) :
			$child = new wpdeals_deals_variation( $child_id, $this->id, $this->deal_custom_fields );
		else :
			$child = new wpdeals_deals( $child_id );
		endif;
		return $child;
	}
		...
		if ($this->managing_stock()) :
			...
			if (!$this->is_in_stock()) :
				...
				update_post_meta($this->id, '_is_expired', 'yes');
                                $wpdeals->clear_deals_transients( $this->id ); // Clear transient
			endif;
	...
	function get_title() {
		$this->get_post_data();
		return apply_filters('wpdeals_deals_title', apply_filters('the_title', $this->post->post_title), $this);
	}
	...
	function add_to_cart_url() {
		...
		$url = $wpdeals->nonce_url( 'add_to_cart', $url );
		return $url;
	}
	...
	function get_availability() {
		...
		if (!$this->managing_stock()) :
			if ($this->is_in_stock()) :
				//$availability = __('In stock', 'wpdeals'); /* Lets not bother showing stock if its not managed and is available */
			else :
				$availability = __('Out of stock', 'wpdeals');
				$class = 'out-of-stock';
			endif;
		else :
			if ($this->is_in_stock()) :
				if ($this->get_total_stock() > 0) :
					$availability = __('In stock', 'wpdeals');
					
					if ($this->backsales_allowed()) :
						if ($this->backsales_require_notification()) :
							$availability .= ' &ndash; '.$this->_stock.' ';
							$availability .= __('available', 'wpdeals');
							$availability .= __(' (backsales allowed)', 'wpdeals');
						endif;
					else :
						$availability .= ' &ndash; '.$this->_stock.' ';
						$availability .= __('available', 'wpdeals');
					...
					if ($this->backsales_allowed()) :
						if ($this->backsales_require_notification()) :
							$availability = __('Available on backorder', 'wpdeals');
						else :
							$availability = __('In stock', 'wpdeals');
						endif;
					else :
	...
	function is_visible( $single = false ) {
		...
		if (get_option('wpdeals_hide_out_of_stock_items') == 'yes') :
			if (!$this->is_expired())
                                $single = false;
                        else
                                $single = true;
	...
	function get_price_html() {
		...
		if ($this->is_type('grouped')) :
			...
			$price .= '<span class="from">' . __('From:', 'wpdeals') . ' </span>' . wpdeals_price($min_price);	
			
			$price = apply_filters('wpdeals_grouped_price_html', $price, $this);
				
		elseif ($this->is_type('variable')) :
			
			if ( !$this->min_variation_price || $this->min_variation_price !== $this->max_variation_price ) $price .= '<span class="from">' . __('From:', 'wpdeals') . ' </span>';
			
			$price .= wpdeals_price($this->get_price());
			
			$price = apply_filters('wpdeals_variable_price_html', $price, $this);
			...
			if ($this->_base_price > 0 && $this->_discount_price > 0) :
				if (isset($this->_discount_price) && isset($this->_base_price) && $this->_base_price > $this->_discount_price ) :
					...
					$price .= '<del>'.wpdeals_price( $this->_base_price ).'</del> <ins>'.wpdeals_price($this->get_sale()).'</ins>';
					
					$price = apply_filters('wpdeals_discount_price_html', $price, $this);
					
				else :
				
					$price .= wpdeals_price($this->get_price());
					
					$price = apply_filters('wpdeals_price_html', $price, $this);
					
				endif;
				
			elseif ($this->_base_price == 0 || $this->_base_price === '' || $this->_discount_price == 0 || $this->_discount_price === '') :
				
                                $price = __('Free!', 'wpdeals');  

                                $price = apply_filters('wpdeals_free_price_html', $price, $this);
	...
	function get_sale_html() {
                ...
                $sale .= wpdeals_price($this->get_sale());

                $sale = apply_filters('wpdeals_sale_html', $sale, $this);
	...
	function get_rating_html( $location = '' ) {
		...
		$star_size = apply_filters('wpdeals_star_rating_size'.$location, 16);

		if ( false === ( $average_rating = get_transient( $this->id . '_wpdeals_average_rating' ) ) ) :
			...
			set_transient( $this->id . '_wpdeals_average_rating', $average_rating );
		
		endif;

		if ( $average_rating>0 ) :
			return '<div class="star-rating" title="'.sprintf(__('Rated %s out of 5', 'wpdeals'), $average_rating).'"><span style="width:'.($average_rating*$star_size).'px"><span class="rating">'.$average_rating.'</span> '.__('out of 5', 'wpdeals').'</span></div>';
		else :
			return '';
		endif;
	}
	...
	function get_related( $limit = 5 ) {
		...
		// Meta query
		$meta_query = array();
                $meta_query[] = $wpdeals->query->stock_status_meta_query();
	...
	function list_attributes() {
		...
		if (get_option('wpdeals_enable_dimension_deals_attributes')=='yes') :
			...
			if ($this->get_weight()) $weight = $this->get_weight() . get_option('wpdeals_weight_unit');
			
			if (($length && $width && $height)) $dimensions = $length . get_option('wpdeals_dimension_unit') . ' x ' . $width . get_option('wpdeals_dimension_unit') . ' x ' . $height . get_option('wpdeals_dimension_unit');
		...
		if (sizeof($attributes)>0 || ($show_dimensions && $has_dimensions)) :
			...
			if (($show_dimensions && $has_dimensions)) :
				...
				if ($weight) :
					...
					echo '<tr class="';
					if ($alt==1) echo 'alt';
					echo '"><th>'.__('Weight', 'wpdeals').'</th><td>'.$weight.'</td></tr>';
				...
				if ($dimensions) :
					...
					echo '<tr class="';
					if ($alt==1) echo 'alt';
					echo '"><th>'.__('Dimensions', 'wpdeals').'</th><td>'.$dimensions.'</td></tr>';
			...
			foreach ($attributes as $attribute) :
				...
				echo '<tr class="';
				if ($alt==1) echo 'alt';
				echo '"><th>'.$wpdeals->attribute_label( $attribute['name'] ).'</th><td>';
	...
	/**
        ...
        function get_image( $size = 'store_thumbnail' ) {
            ...
            if (has_post_thumbnail($this->id)) :
                    ...
                    elseif (($parent_id = wp_get_post_parent_id( $this->id )) && has_post_thumbnail($parent_id)) :
                            ...
                            echo '<img src="'.$wpdeals->plugin_url(). '/wpdeals-assets/images/placeholder.png" alt="Placeholder" width="'.$wpdeals->get_image_size('store_thumbnail_image_width').'" height="'.$wpdeals->get_image_size('store_thumbnail_image_height').'" />'; 
                    endif;
        }
        ...
        function get_discount() {
            ...
            return apply_filters( 'wpdeals_discount_price', $disc_percent);
        ...
        function get_save_price() {
            ...
            return apply_filters( 'wpdeals_save_price', $save_price);
        ...
        function get_purchase(){
            global $wpdb;
            $results = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM ".$wpdb->prefix."wpdeals_permissions WHERE deal_id = '%s';", $this->id) );
            return apply_filters('wpdeals_purchase_deals', $results);
        }
