 ...
 * Customer
 * 
 * The WPDeals customer class handles storage of the current customer's data, such as location.
 *
 * @class wpdeals_customer
 * @package		WPDeals
 * @category	Class
 * @author		Tokokoo
 */
class wpdeals_customer {
	...
	function get_downloadable_deals() {
		...
		if (is_user_logged_in()) :
			...
			$results = $wpdb->get_results( $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."wpdeals_permissions WHERE user_id = '%s' AND vouchers = '';", get_current_user_id()) );
			...
			if ($results) foreach ($results as $result) :
				...
				if ($result->order_id>0) :
					...
					if ($deal_post->post_type=='deal-variations') :
						$_deals = new wpdeals_deals_variation( $result->deal_id );
					else :
						$_deals = new wpdeals_deals( $result->deal_id );
					endif;					
		...
		return apply_filters('wpdeals_customer_get_downloadable_deals', $downloads);
	...
	function get_coupons_deals() {
		...
		return apply_filters('wpdeals_customer_get_coupon_deals', $coupons);
