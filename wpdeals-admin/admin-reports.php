...
function wpdeals_reports() {

	$current_tab = (isset($_GET['tab'])) ? $_GET['tab'] : 'sales';
	$current_chart = (isset($_GET['chart'])) ? $_GET['chart'] : 0;
	
	$charts = array(
		__('sales', 'wpdeals') => array(
			array(
				'title' => __('Overview', 'wpdeals'),
				'description' => '',
				'hide_title' => true,
				'function' => 'wpdeals_sales_overview'
			),
			array(
				'title' => __('Sales by day', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_daily_sales'
			),
			array(
				'title' => __('Sales by month', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_monthly_sales'
			),
			array(
				'title' => __('Deal Sales', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_deals_sales'
			),
			array(
				'title' => __('Top sellers', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_top_sellers'
			),
			array(
				'title' => __('Top earners', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_top_earners'
			),
			array(
				'title' => __('Export to CSV (this month)', 'wpdeals'),
				'description' => '',
				'function' => 'wpdeals_export_to_csv_this_month'
			)
		),
		__('customers', 'wpdeals') => array(
			array(
				'title' => __('Overview', 'wpdeals'),
				'description' => '',
				'hide_title' => true,
				'function' => 'wpdeals_customer_overview'
			),
		)
	);
    ?>
	<div class="wrap wpdeals">
		<div class="icon32 icon32-wpdeals-reports" id="icon-wpdeals"><br></div><h2 class="nav-tab-wrapper wpdeals-nav-tab-wrapper">
			<?php
				foreach ($charts as $name => $value) :
					echo '<a href="'.admin_url('admin.php?page=wpdeals_reports&tab='.$name).'" class="nav-tab ';
					if($current_tab==$name) echo 'nav-tab-active';
					echo '">'.ucfirst($name).'</a>';
				endforeach;
			?>
			<?php do_action('wpdeals_reports_tabs'); ?>
		</h2>
		
		<?php if (sizeof($charts[$current_tab])>1) : ?><ul class="subsubsub"><li><?php
			$links = array();
			foreach ($charts[$current_tab] as $key => $chart) :
				$link = '<a href="admin.php?page=wpdeals_reports&tab='.$current_tab.'&amp;chart='.$key.'" class="';
 ...
 */
function wpdeals_weekend_area_js() {
	?>
        ...
        if (item) {
            if (previousPoint != item.dataIndex) {
                ...
                if (item.series.label=="Sales amount") {
                	...
                	showTooltip(item.pageX, item.pageY, item.series.label + ": " + "<?php echo get_wpdeals_currency_symbol(); ?>" + y);
 ...
 */
function wpdeals_datepicker_js() {
	global $wpdeals;
	?>
	var dates = jQuery( "#from, #to" ).datepicker({
		...
		showOn: "button",
		buttonImage: "<?php echo $wpdeals->plugin_url(); ?>/wpdeals-assets/images/calendar.png",
...
function wpdeals_sales_overview() {
	...
	global $start_date, $end_date, $wpdeals, $wpdb;
	...
	?>
	<div id="poststuff" class="wpdeals-reports-wrap">
		<div class="wpdeals-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e('Total sales', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Total sales', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo $total_sales. ' ('.$order_items.__(' items', 'wpdeals').')'; else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Average order total', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales/$total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Average order items', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo number_format($order_items/$total_sales, 2); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Last 5 sales', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<?php
					if ($sales) :
						$count = 0;
						...
						foreach ($sales as $order) :
							...
							<li>
								...
								<small>'.sizeof($this_order->items).' '._n('item', 'items', sizeof($this_order->items), 'wpdeals').' <span class="order-cost">'.__('Total:', 'wpdeals') . ' ' . wpdeals_price($this_order->order_total).'</span> <span class="order-customer">'.$customer.'</span></small>
							</li>';
		...
		<div class="wpdeals-reports-main">
			<div class="postbox">
				<h3><span><?php _e('This months sales', 'wpdeals'); ?></span></h3>
	...
	<script type="text/javascript">
		jQuery(function(){
			...
			<?php wpdeals_weekend_area_js(); ?>
			<?php wpdeals_tooltip_js(); ?>
		});
	</script>
	<?php
}
...
function wpdeals_daily_sales() {
	...
	<form method="post" action="">
		<p><label for="from"><?php _e('From:', 'wpdeals'); ?></label> <input type="text" name="start_date" id="from" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $start_date) ); ?>" /> <label for="to"><?php _e('To:', 'wpdeals'); ?></label> <input type="text" name="end_date" id="to" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $end_date) ); ?>" /> <input type="submit" class="button" value="<?php _e('Show', 'wpdeals'); ?>" /></p>
	</form>
	
	<div id="poststuff" class="wpdeals-reports-wrap">
		<div class="wpdeals-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e('Total sales in range', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			...
			<div class="postbox">
				<h3><span><?php _e('Average order total in range', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales/$total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Average order items in range', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo number_format($order_items/$total_sales, 2); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
		...
		<div class="wpdeals-reports-main">
			<div class="postbox">
				<h3><span><?php _e('Sales in range', 'wpdeals'); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
				</div>
			</div>
...
function wpdeals_monthly_sales() {
	...
	?>
	<form method="post" action="">
		<p><label for="show_year"><?php _e('Year:', 'wpdeals'); ?></label> 
		<select name="show_year" id="show_year">
			<?php
				for ($i = $first_year; $i <= date('Y'); $i++) printf('<option value="%u" %u>%u</option>', $i, selected($current_year, $i, false), $i);
			?>
		</select> <input type="submit" class="button" value="<?php _e('Show', 'wpdeals'); ?>" /></p>
	</form>
	<div id="poststuff" class="wpdeals-reports-wrap">
		<div class="wpdeals-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e('Total sales for year', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			...
			<div class="postbox">
				<h3><span><?php _e('Average order total for year', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo wpdeals_price($total_sales/$total_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Average order items for year', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo number_format($order_items/$total_sales, 2); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
		...
		<div class="wpdeals-reports-main">
			<div class="postbox">
				<h3><span><?php _e('Monthly sales for year', 'wpdeals'); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
				</div>
			</div>
...
function wpdeals_top_sellers() {
	...
	global $start_date, $end_date, $wpdeals;
			...
			<?php
				...
				foreach ($found_deals as $deal_id => $sales) :
					...
					if ($deal) :
						...
						$deal_name = __('Deal does not exist', 'wpdeals');
					endif;
...
function wpdeals_top_earners() {
	...
	<table class="bar_chart">
		<thead>
			<tr>
				<th><?php _e('Deal', 'wpdeals'); ?></th>
				<th colspan="2"><?php _e('Sales', 'wpdeals'); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				foreach ($found_deals as $deal_id => $sales) :
					...
					if ($deal) :
						...
						$deal_name = __('Deal no longer exists', 'wpdeals');
					endif;
					
					$sales_link = admin_url('edit.php?s&post_status=all&post_type=deals-sales&action=-1&s=' . urlencode($deal->post_title) . '&deals_sales_status=completed,processing,on-hold');
					
					echo '<tr><th>'.$deal_name.'</th><td width="1%"><span>'.wpdeals_price($sales).'</span></td><td class="bars"><a href="'.$sales_link.'" style="width:'.$width.'%">&nbsp;</a></td></tr>';
				endforeach; 
			?>
		</tbody>
	</table>
...
function wpdeals_deals_sales() {
	...
	if ($chosen_deals_id) :
		...
		$chosen_deals = new wpdeals_deals( $chosen_deals_id );
	...
	?>
	<form method="post" action="">
		<p><label for="from"><?php _e('Deal:', 'wpdeals'); ?></label>
		<select name="deal_id" id="deal_id">
			<?php
				echo '<option value="">'.__('Choose an deals&hellip;', 'wpdeals').'</option>';
		...
		<tbody>
			<?php
				if (sizeof($deals_sales)>0) foreach ($deals_sales as $date => $sales) :
					...
					<td width="1%"><span>'.$sales.'</span><span class="alt">'.wpdeals_price($deal_totals[$date]).'</span></td>
					<td class="bars">
						<span style="width:'.$width.'%">&nbsp;</span>
						<span class="alt" style="width:'.$width2.'%">&nbsp;</span>
					</td></tr>';
				endforeach; else echo '<tr><td colspan="3">'.__('No sales :(', 'wpdeals').'</td></tr>';
			?>
		</tbody>
	</table>
	<?php
...
function wpdeals_customer_overview() {
	...
	<div id="poststuff" class="wpdeals-reports-wrap">
		<div class="wpdeals-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e('Total customers', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customers>0) echo $total_customers; else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Total customer sales', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customer_sales>0) echo wpdeals_price($total_customer_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Total guest sales', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_guest_sales>0) echo wpdeals_price($total_guest_sales); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e('Average sales per customer', 'wpdeals'); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customer_sales>0 && $total_customers>0) echo number_format($total_customer_sales/$total_customers, 2); else _e('n/a', 'wpdeals'); ?></p>
				</div>
			</div>
		</div>
		<div class="wpdeals-reports-main">
			<div class="postbox">
				<h3><span><?php _e('Signups per day', 'wpdeals'); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
				</div>
			</div>
